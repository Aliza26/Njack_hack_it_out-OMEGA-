...
public class RTEditorActivity extends RTEditorBaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //mSubjectField.setText(sub);
        //mRTMessageField.setText(message);
        //mRTSignatureField.setText(signature);
        ...
        if (savedInstanceState == null) {
            ...
            subject = savedInstanceState.getString(PARAM_SUBJECT, "aaa");
        ...
        RTToolbar rtToolbar0 = findViewById(com.onegravity.rteditor.R.id.rte_toolbar);
        if (rtToolbar0 != null) {
            mRTManager.registerToolbar(toolbarContainer, rtToolbar0);
        }

        // register toolbar 1 (if it exists)
        RTToolbar rtToolbar1 = findViewById(com.onegravity.rteditor.R.id.rte_toolbar_character);
        if (rtToolbar1 != null) {
            mRTManager.registerToolbar(toolbarContainer, rtToolbar1);
        }

        // register toolbar 2 (if it exists)
        RTToolbar rtToolbar2 = findViewById(com.onegravity.rteditor.R.id.rte_toolbar_paragraph);
        if (rtToolbar2 != null) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.load) {
            ...
            String subject = mSubjectField.getText().toString();
            String message = mRTMessageField.getText().toString();
            String signature = mRTSignatureField.getText().toString();


            // validating if the text fields are empty or not.
            if (subject.isEmpty() && message.isEmpty() && signature.isEmpty()) {
                Toast.makeText(RTEditorActivity.this, "Please enter all the data..", Toast.LENGTH_SHORT).show();
                return false;
            }

            // on below line we are calling a method to add new
            // course to sqlite data and pass all our values to it.
           // Toast.makeText(this, "before save:"+sub+":"+message+":"+signature, Toast.LENGTH_SHORT).show();
            dbHandler.adNewDetails(subject,message,signature);



            // after adding the data we are displaying a toast message.
            Toast.makeText(RTEditorActivity.this, "Notes has been saved.", Toast.LENGTH_SHORT).show();
            mSubjectField.setText("");
            mRTMessageField.setText("");
            mRTSignatureField.setText("");
